...
-- Create user stats table if not exists
DO $$
BEGIN
...
-- Create watch sessions table if not exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'watch_sessions') THEN
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'videos' AND rowsecurity = true) THEN
        ALTER TABLE public.videos ENABLE ROW LEVEL SECURITY;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_stats' AND rowsecurity = true) THEN
        ALTER TABLE public.user_stats ENABLE ROW LEVEL SECURITY;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'watch_sessions' AND rowsecurity = true) THEN
        ALTER TABLE public.watch_sessions ENABLE ROW LEVEL SECURITY;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_preferences' AND rowsecurity = true) THEN
        ALTER TABLE public.user_preferences ENABLE ROW LEVEL SECURITY;
    END IF;
END $$;

-- Profiles policies (conditional creation)
DO $$
BEGIN
...
-- Videos policies (conditional creation)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'videos' AND policyname = 'Users can read own videos') THEN
        CREATE POLICY "Users can read own videos" ON public.videos
          FOR SELECT USING (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'videos' AND policyname = 'Users can insert own videos') THEN
        CREATE POLICY "Users can insert own videos" ON public.videos
          FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'videos' AND policyname = 'Users can update own videos') THEN
        CREATE POLICY "Users can update own videos" ON public.videos
          FOR UPDATE USING (auth.uid() = user_id)
          WITH CHECK (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'videos' AND policyname = 'Users can delete own videos') THEN
        CREATE POLICY "Users can delete own videos" ON public.videos
          FOR DELETE USING (auth.uid() = user_id);
    END IF;
END $$;

-- User stats policies (conditional creation)
DO $$
BEGIN
...
-- Watch sessions policies (conditional creation)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'watch_sessions' AND policyname = 'Users can read own watch sessions') THEN
        CREATE POLICY "Users can read own watch sessions" ON public.watch_sessions
          FOR SELECT USING (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'watch_sessions' AND policyname = 'Users can insert own watch sessions') THEN
        CREATE POLICY "Users can insert own watch sessions" ON public.watch_sessions
          FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'watch_sessions' AND policyname = 'Users can update own watch sessions') THEN
